...
on run argv
    ...
    if testMode is not "flow" and testMode is not "entry" then error "unknown test mode"
...
assert_entry_events success 'lock,activate,open,wait,backup,set-command,paste,send,restore,release'
